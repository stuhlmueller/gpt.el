...
Use `gpt-script-path' as the executable and pass the other arguments as a list."
  (let ((process (start-process "gpt-process" output-buffer python-interpreter gpt-script-path gpt-openai-key gpt-openai-engine gpt-openai-max-tokens gpt-openai-temperature prompt-file)))
    process))
